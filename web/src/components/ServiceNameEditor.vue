...
const props=defineProps<{id:string;name:string;expectedName?:string}>();
...
async function open(){value.value=props.name;base.value=props.expectedName??props.name;editing.value=true;error.value='';await nextTick();input.value?.focus();input.value?.select();}
async function save(){if(busy.value)return;busy.value=true;error.value='';message.value='Сохраняем имя…';try{
 const r=await submitOp('service.rename',{service_id:props.id,display_name:value.value.trim(),expected_name:base.value});
...
<template><div class="inline-rename">
 ...
 <form v-else @submit.prevent="save"><label :for="`service-name-${id}`">Новое имя сервиса</label><input ref="input" :id="`service-name-${id}`" v-model="value" maxlength="255" required :disabled="busy" autocomplete="off"/>
