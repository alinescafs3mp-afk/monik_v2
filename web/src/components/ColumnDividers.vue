...
<template>
  <div v-if="layout.enabled" class="column-dividers" :class="{'is-resizing':layout.resizing}">
    <span v-for="(left,i) in layout.positions" :key="i" class="column-divider" role="separator"
      :tabindex="keyboard ? 0 : -1" :class="{'remote-selected':layout.remoteIndex===i}" draggable="false" aria-orientation="vertical" :aria-label="`Ширина ${columnLabels[i]}`"
      ...
      @keydown="layout.key($event,i)" @contextmenu.prevent @dragstart.prevent @click.stop.prevent />
  </div>
</template>
