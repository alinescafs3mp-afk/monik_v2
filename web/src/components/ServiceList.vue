...
<template>
  <div class="service-list" :class="{'service-list-compact': compact}">
    ...
    <div v-for="s in visible" :key="s.id" class="service-row">
      ...
      <p class="service-outcome" :title="s.summary" :class="{ err: !forceStale && ['app_fail','http_error','transport_fail'].includes(s.state) }">{{ s.summary }} <small v-if="s.maintenance_active"> · Обслуживание</small><span v-if="forceStale && s.fresh"> · устарело</span></p>
