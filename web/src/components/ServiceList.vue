...
<template>
  <div class="service-list" :class="{'service-list-compact': compact}">
    ...
    <div v-for="s in visible" :key="s.id" class="service-row">
      ...
      <p v-if="!compact && serviceHint(s)" class="muted">{{serviceHint(s)}}</p><template v-if="!compact"><ServiceNameEditor v-if="!locked" :id="String(s.id)" :name="String(s.display_name||s.url)" :expected-name="String(s.display_name??'')"/><div class="service-choices"><ServiceMonitorToggle :service="s" :locked="locked"/><ServiceOverviewToggle :service="s"/></div><button v-if="configureInPlace" type="button" @click="emit('configure',String(s.id))">Настроить запрос</button><router-link v-else :to="{path:`/machines/${encodeURIComponent(s.agent_id)}`,query:{service:s.id},hash:'#check-editor'}">Настроить запрос</router-link></template>
