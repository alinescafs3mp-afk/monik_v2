...
import {acknowledgementFilter} from '../monitoring';
...
const acknowledgement=ref(acknowledgementFilter(route.query.acknowledgement));
watch(()=>route.query.acknowledgement,v=>{acknowledgement.value=acknowledgementFilter(v)});
watch(acknowledgement,v=>{void router.replace({query:{...route.query,acknowledgement:v}});});
...
async function load(){
 ...
 const params=new URLSearchParams({...range,state:state.value,severity:severity.value,metric:metric.value,entity_id:entity.value,acknowledgement:acknowledgement.value,limit:'100'});
...
watch([mode,hours,state,severity,metric,acknowledgement],()=>{++generation;void refresh();});
...
async function ack(id:string,unread=false){
 if(pending.value||mode.value==='history')return;pending.value=id;
 try{const r=await submitOp(unread?'incident.unacknowledge':'incident.acknowledge',{incident_id:id});emit('toast',unread?'Инцидент снова непрочитан. История действий сохранена.':'Прочтение подтверждено. Это не означает восстановление.',`/operations/${r.op?.operation_id}`);await refresh();}
...
<template>
 <div>
  ...
  <div class="row">
   <label>Состояние <select v-model="state" aria-label="Состояние"><option value="open">Открытые</option><option value="all">Все</option><option value="resolved">Восстановленные</option><option value="policy_changed">Правило изменено</option><option value="interrupted">Наблюдение прервано</option><option value="confirmed">Подтверждённые</option><option value="pending">Ожидают подтверждения</option></select></label>
   <label>Прочтение <select v-model="acknowledgement" aria-label="Прочтение"><option value="all">Все</option><option value="unread">Непрочитанные</option><option value="read">Прочитанные</option></select></label>
  ...
  <div v-if="incs.length" class="table-wrap"><table><thead><tr><th>Начало / завершение</th><th>Объект</th><th>Метрика</th><th>Состояние</th><th>Причина</th><th>Прочтение</th></tr></thead><tbody>
   <tr v-for="i in incs" :key="i.id"><td>{{ instant(i.opened_at) }}<small v-if="i.resolved_at"> → {{ instant(i.resolved_at) }}</small></td>
    <td><router-link v-if="i.agent_id" :to="{path:'/machines/'+encodeURIComponent(i.agent_id),query:{at:i.opened_at,hours:1,...(i.entity_type==='service'?{service:i.entity_id}:{})},hash:i.entity_type==='service'?'#services':''}" title="Текущее имя объекта; открывается исторический интервал">{{i.display_name||i.entity_id}}</router-link><span v-else>{{i.display_name||i.entity_id}}</span><small>{{i.entity_type==='service'?'Сервис':'Машина'}}</small></td>
    <td>{{ i.metric }}</td><td>{{ incidentStateLabel(i.status) }} · {{ i.severity==='critical'?'Критическая':'Предупреждение' }}</td><td>{{ i.reason }}<small v-if="i.rule_version">Правило #{{i.rule_version}}</small><small v-if="i.maintenance_observed">Наблюдался во время обслуживания</small><small v-if="i.maintenance_active">Сейчас идёт обслуживание</small></td>
    <td><span v-if="i.acked_at">Прочитан {{ instant(i.acked_at) }}<button :disabled="!!pending||mode==='history'" @click="ack(i.id,true)">Считать непрочитанным</button></span><button v-else :disabled="!!pending||mode==='history'" @click="ack(i.id)">{{ pending===i.id?'Сохраняем…':'Прочитано' }}</button></td>
   </tr></tbody></table></div>
