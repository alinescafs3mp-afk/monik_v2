...
watch(()=>route.hash,h=>{if(h==='#services')tab.value='services';});
...
<template>
 <div>
  ...
  <template v-if="detail">
   ...
   <section v-if="tab==='services'&&detail.discovery" class="panel"><h3>Покрытие обнаружения</h3><p>Слушающих портов: {{detail.discovery.listener_count}} · {{detail.discovery.coverage_complete?'Проход завершён в доступной области':'Проход неполный'}} · {{new Date(detail.discovery.ended_at).toLocaleString()}}</p><p v-if="detail.discovery.truncated" class="data-warning">Достигнут бюджет обнаружения. Отсутствие сервиса в этом проходе не означает его удаление.</p><p v-for="reason in detail.discovery.permission_gaps||[]" :key="reason">{{reason}}</p><details v-if="detail.discovery.unresolved?.length"><summary>Не распознано как HTTP: {{detail.discovery.unresolved.length}}</summary><p v-for="u in detail.discovery.unresolved" :key="u.dial_target">{{u.dial_target}} · {{u.process_name||'Процесс неизвестен'}} · {{u.reason}}</p></details></section>
   <CheckEditor v-show="tab==='services'" :key="detail.agent.id" :detail="detail" :readonly="mode==='history'" @refresh="refresh" @toast="(message,link)=>emit('toast',message,link)"/>
