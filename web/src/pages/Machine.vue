...
watch([hours,mode,()=>route.params.id],(v,old)=>{++generation; exportURL.value='';if(v[0]!==old[0]||v[2]!==old[2])cursor.value='';void refresh();});
const host=computed(()=>mode.value==='history'?point.value?.host:detail.value?.host);
const rows=computed<any[]>(()=>history.value?.host || []);
const trialKind=ref('baseline_http'), trialJSONPath=ref(''), trialJSONValue=ref(''), trialText=ref('');
...
watch(trialService, id => {
 const d=detail.value?.desired_config?.checks?.find((c:any)=>c.service_id===id);
 const service=detail.value?.services?.find((c:any)=>c.id===id);
 trialUrl.value=d?.url || service?.url || ''; trialMethod.value=d?.method || 'GET';
 trialKind.value=d?.kind==='baseline_http' || !d?.kind ? 'baseline_http' : 'http_health';
 trialExpect.value=d?.expected_status?.join(',') || '200';
 trialJSONPath.value=d?.expect_json_path || ''; trialJSONValue.value=d?.expect_json_value || ''; trialText.value=d?.expect_text || '';
});
function checkDraft() {
 const prev=detail.value?.desired_config?.checks?.find((c:any)=>c.service_id===trialService.value) || {};
 const expected=trialExpect.value.split(',').filter(s=>s.trim()).map(s=>Number(s.trim()));
 if(trialKind.value!=='baseline_http' && (!expected.length || expected.some(n=>!Number.isInteger(n)||n<100||n>599))) throw new Error('Коды HTTP должны быть целыми числами от 100 до 599.');
 if(trialMethod.value==='HEAD' && trialKind.value!=='baseline_http' && (trialJSONPath.value || trialText.value)) throw new Error('Проверка тела ответа требует GET.');
 return {...prev, id:prev.id || crypto.randomUUID(), service_id:trialService.value,url:trialUrl.value,method:trialMethod.value,kind:trialKind.value,
  expected_status:trialKind.value==='baseline_http'?[]:expected,expect_json_path:trialKind.value==='baseline_http'?'':trialJSONPath.value,
  expect_json_value:trialKind.value==='baseline_http'?'':trialJSONValue.value,expect_text:trialKind.value==='baseline_http'?'':trialText.value,
  timeout_seconds:2,interval_seconds:5};
}
...
async function trial(){
  ...
  try{
    const r=await submitOp('check.trial',checkDraft(),[String(route.params.id)]);
...
async function saveCheck(){
  ...
  try{
    const r=await submitOp('check.apply',{check:checkDraft(),base_revision:detail.value.agent.desired_revision},[String(route.params.id)]);
    await refresh();
 ...
 <div>
  ...
  <template v-if="detail">
   ...
   <section v-if="tab==='services'" class="panel">
    ...
    <label>Что проверять <select v-model="trialKind"><option value="baseline_http">Ответ HTTP и безопасная сводка</option><option value="http_health">Здоровье приложения по условиям</option></select></label>
    <p class="muted">GET читает не более 64 КиБ. Показываются код, время, Content-Type и известные статусы из JSON/text; HTML и произвольные сообщения не сохраняются. Самоотчёт сервиса не заменяет проверку здоровья.</p>
    <template v-if="trialKind!=='baseline_http'"><label>Ожидаемые коды <input v-model="trialExpect"/></label><label>Поле JSON (например status) <input v-model="trialJSONPath"/></label><label>Ожидаемое значение JSON <input v-model="trialJSONValue"/></label><label>Ожидаемый текст <input v-model="trialText"/></label></template>
