...
<div class="row"><input v-model="planId" placeholder="plan_id" /><button :disabled="!selected.length||pending||!planId" @click="run('rebind.arm',{plan_id:planId})">Вооружить</button><button :disabled="!selected.length||pending||!planId" @click="run('rebind.activate',{plan_id:planId})">Переключить</button><button :disabled="!selected.length||pending||!planId" @click="run('rebind.retire',{plan_id:planId})">Снять план</button></div>
...
<div class="table-wrap"><table><thead><tr><th>Выбор</th><th>Машина</th><th>ОС</th><th>Версия</th><th>Связь</th><th>Конфигурация</th><th>Установка</th></tr></thead>
...
<td>{{ r.managed_ready?'управляемая':'неуправляемая' }}</td></tr></tbody></table></div></div></template>
