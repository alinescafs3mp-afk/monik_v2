...
export function orderedServices(services: any[] = [], stateOf: (s:any)=>string = s=>s.state) {
  return [...services].sort((a, b) => (severity[stateOf(a)] ?? 2) - (severity[stateOf(b)] ?? 2)
...
export function overviewPriority(machine: any, metricsFresh: boolean, stateOf:(s:any)=>string = s=>s.state): number {
  ...
  const selectedProblem = (machine?.services || []).some((s:any) => s.pinned !== false && stateOf(s) !== 'paused' && failed.includes(stateOf(s)));
  if (metricProblem || selectedProblem) return 2;
  if ((machine?.services || []).some((s:any)=>s.pinned!==false&&stateOf(s)==='stale')) return 1;
  if (['unreachable','stale'].includes(machine?.state) || (!metricsFresh && machine?.state === 'ok')) return 1;
  return 0;
}
export function orderOverview<T extends {name?:string;id?:string}>(machines:T[], isFresh:(m:T)=>boolean, stateOf:(s:any)=>string = s=>s.state):T[] {
  return [...machines].sort((a,b)=>overviewPriority(b,isFresh(b),stateOf)-overviewPriority(a,isFresh(a),stateOf)
...
export function serviceColumnCapacity(width:number,columnWidth=200):number {
 ...
 if(!Number.isFinite(columnWidth)||columnWidth<=0)columnWidth=200;
