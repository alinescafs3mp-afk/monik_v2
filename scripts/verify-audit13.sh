...
make dist -o ui GO="$GO" VERSION="${VERSION:-0.1.0-audit13}" 2>&1 | tee "$out/build.log"
...
"$GO" test -race -shuffle=on -count=10 -run '^TestV13' ./internal/agent/runtime ./internal/agent/spool ./internal/storage ./internal/server ./internal/jsonutil ./internal/agent/checks 2>&1 | tee "$out/repeated-regressions.log"
"$GO" test -run '^$' -fuzz '^FuzzV13StrictJSON$' -fuzztime=15s -parallel=2 ./internal/jsonutil 2>&1 | tee "$out/json-fuzz.log"
