...
def exercise_inventory_profiles(page, context, base, directory, output, results):
    ...
    detail=context.request.get(base+'/api/v1/agents/audit-host')
    assert detail.ok, f'machine API failed before services tab: {detail.status} {detail.text()}'
    ...
    installer=page.locator('.installer-panel')
    expect(installer.get_by_role('heading',name='Один файл. Один запуск.',exact=True)).to_be_visible()
    expect(installer.get_by_role('button',name='Скачать агент: linux-amd64',exact=True)).to_be_disabled()
    assert not page.locator('.advanced-enrollment').evaluate('(node)=>node.open')
    expect(installer).to_contain_text('sudo ./monik-agent')
    results.append('single-file setup is the primary path; missing template is explicit, not a fake ready download; advanced paths are collapsed')
